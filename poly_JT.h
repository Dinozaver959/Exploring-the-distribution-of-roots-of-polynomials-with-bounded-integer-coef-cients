...
namespace my_space { // START USING NAMESPACE STD
...
while (N >= 1){ // Main loop
    ...
    // Return with failure if no convergence with 100 shifts

    if (jj > 100) {
        cout << "\nFailure. No convergence after 100 shifts. Program terminated.\n";
        *Degree -= N;
        break;
    } // End if (jj > 20)
